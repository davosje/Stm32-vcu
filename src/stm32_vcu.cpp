...
static void Ms200Task(void) {
  ...
  if (targetChgint == ChargeInterfaces::Chademo && chargeModeDC)
    selectedChargeInt->Task200Ms();
  // In case of the LIM we want to send it all the time if lim in use
  if ((targetChgint == ChargeInterfaces::i3LIM) ||
      (targetChgint == ChargeInterfaces::Unused) ||
      (targetChgint == ChargeInterfaces::CPC) ||
      (targetChgint == ChargeInterfaces::Foccci))
    selectedChargeInt->Task200Ms();
  // and just to be thorough ...
  if (targetChgint == ChargeInterfaces::Unused)
    selectedChargeInt->Task200Ms();
  ...
  */
  ...
  if (opmode == MOD_RUN) {
    ...
    if (Param::GetInt(Param::GPA1Func) == IOMatrix::VAC_SENSOR ||
      ...
      // if brkVacThresh > BrkVacHyst then sensor reads higher with more vacuum
...
static void Ms100Task(void) {
  ...
  Param::SetFloat(
      Param::tmphs,
      selectedInverter
          ->GetInverterTemperature()); // send inverter temp to web interface
  Param::SetFloat(
      Param::tmpm,
      selectedInverter
          ->GetMotorTemperature()); // send motor temp to web interface
  Param::SetFloat(
      Param::InvStat,
      selectedInverter
          ->GetInverterState()); // update inverter status on web interface
  Param::SetFloat(
      Param::INVudc,
                ...
                // external pin io
  {
    ...
    if (opmode != MOD_RUN)
      chargeMode = true; // set charge mode to true to bring up hv
  ...
  if (IOMatrix::GetPinIn(IOMatrix::HVREQ) != &DigIo::dummypin) {
    ExtHVreq = IOMatrix::GetPinIn(IOMatrix::HVREQ)
                   ->Get(); // Read IO pin to determine if there is an external
                            // HV request
    if (ExtHVreq)
      Param::SetInt(Param::chgtyp, DCEXT);
  }
  ...
  if (IOMatrix::GetPinIn(IOMatrix::HVIL) != &DigIo::dummypin) {
    CheckHVIL = IOMatrix::GetPinIn(IOMatrix::HVIL)->Get();
    if (CheckHVIL > 0) {
      HVILok = 1;
    } else {
      HVILok = 0;
  ...
  if (opmode == MOD_RUN && Param::GetInt(Param::ShiftLock) == 1) {
    ...
    IOMatrix::GetPinOut(IOMatrix::SHIFTLOCKNO)->Clear(); // Shift Lock Out Off
  }
}

static void ControlCabHeater(int opmode) {
  // Only run heater in run mode if enabled or timer set, also run is mode is
...
static void Ms10Task(void) {
  ...
  {
    torquePercent = utils::ProcessThrottle(
    ...
    if (torquePercent < 0 &&
        Param::GetInt(Param::Inverter) != InvModes::OpenI) {
      if (Param::GetInt(Param::reversemotor) == 0) {
        rollingDirection = previousSpeed >= 0 ? 1 : -1;
      } else {
        rollingDirection = previousSpeed >= 0 ? -1 : 1;
      }
      // When rolling backward while in forward gear, apply POSITIVE torque to
      // slow down backward motion Vice versa when in reverse gear and rolling
      // forward.
  ...
  if (opmode == MOD_RUN ||
      opmode == MOD_CHARGE) // only set rev counter when in RUN or CHARGE
  {
    selectedVehicle->SetRevCounter(
        ABS(speed)); // ABS allowed here to keep number from rolling over.
  }
  ...
  stt |= Param::GetInt(Param::pot) <= Param::GetInt(Param::potmin)
             ? STAT_NONE
             : STAT_POTPRESSED;
  ...
  case MOD_PRECHARGE:
    ...
    if (initbyCharge && !chargeMode)
      opmode = MOD_OFF; // These two statements catch a precharge hang from
                        // either start mode or run mode.
...
static void SetCanFilters() {
  ...
  if (Param::GetInt(Param::ShuntType) == 1 ||
    ...
    ISA::RegisterCanMessages(shunt_can); // select isa shunt
  if (Param::GetInt(Param::ShuntType) == 2)
...
void Param::Change(Param::PARAM_NUM paramNum) {
  ...
  if (ChgSet == 1) {
    seconds = Param::GetInt(Param::Set_Sec); // only update these params if
